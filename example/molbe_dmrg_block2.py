...
for a in seps:
    ...
    # NOTE: This does *not* delete the log files `dmrg.conf` and `dmrg.out`
    # for each frag, # which can still be found in `/scratch/`.
...
# The following # input, for example, yields a 60 sweep schedule
# which uses the two-dot algorithm from sweeps 0-49, and the one-dot algo from 50-60.
...
# To make sure the calculation is proceeding as expected,
# make sure to check `[scratch]/dmrg.conf`
# and `[scratch]/dmrg.out`, which are the fragment DMRG inputs and outputs,
# respectively, used by `block2`.
