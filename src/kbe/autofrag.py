...
def autogen(
    ...
    fragmentations.
    It automatically detects branched chemical chains and ring systems and partitions
    accordingly.  For efficiency, it only checks two atoms for connectivity
    (chemical bond) if they are within 3.5 Angstrom.
    This value is hardcoded as normdist. Two atoms are defined as bonded
    if they are within 1.8 Angstrom (1.2 for Hydrogen atom).
    This is also hardcoded as bond & hbond. Neighboring unitcells are
    used in the fragmentation, exploiting translational symmetry.
        ...
        for idx, i in enumerate(kpt):
            if not i == 1:
                ...
                if n_ > kpt[idx]:
                    ...
                    print(
                        "Fragment cell larger than all k-points combined is not "
                        "supported",
                        flush=True,
                    )
                    sys.exit()
