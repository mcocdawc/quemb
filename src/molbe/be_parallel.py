...
def run_solver(
    ...
    tuple
        ...
        one-particle and two-particle reduced density matrices, and optionally
        the fragment energy.
    """
...
def be_func_parallel(
    ...
    in BE density match. For selected CI solvers, this function exposes thresholds
    used in selected CI calculations (hci_cutoff, ci_coeff_cutoff, select_cutoff).
    ...
    only_chem : bool, optional
        Whether to perform chemical potential optimization only. Refer to
        bootstrap embedding literature. Defaults to False.
    eeval : bool, optional
    ...
    -------
    float or tuple
        Depending on the parameters, returns the error norm or a tuple containing the
        error norm, error vector, and the computed energy.
    """
    import os
